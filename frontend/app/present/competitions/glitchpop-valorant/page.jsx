      ...
      <div className="relative z-10 max-w-7xl mx-auto p-8">
          ...
          <div className="lg:col-span-7 space-y-8">
            <div className="inline-block border-l-4 border-red-500 pl-6">
              ...
              <p className="text-gray-400 max-w-md">
                Filter top 8/16 teams via online knockout for a 3-day LAN final
                at IIT Indore. Blend your style and experience on a global,
                competitive stage.
              </p>
            </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() =>
                router.push("/present/registration?competition=valorant")
              }
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <RoleCard
            icon={<Target />}
            role="Duelist"
            desc="Self-sufficient fraggers."
          />
          <RoleCard
            icon={<Shield />}
            role="Sentinel"
            desc="Defensive experts."
          />
          <RoleCard
            icon={<Users />}
            role="Controller"
            desc="Slice up dangerous territory."
          />
          <RoleCard
            icon={<Zap />}
            role="Initiator"
            desc="Challenge angles & set up teams."
          />
        </div>
        ...
        <div className="mt-24 border-t border-white/10 pt-8 flex justify-between items-end">
          <div>
            <Link
              ...
              className="text-xs text-gray-500 hover:text-white transition-colors uppercase tracking-widest flex items-center gap-2 underline"
            >
              <Timer className="w-4 h-4" /> View Full Rulebook
            </Link>
          </div>
          <div className="text-right">
            <p className="text-4xl font-black text-white/10 uppercase">
              GLITCHPOP // IITI
            </p>
          </div>
        </div>
...
function StatBox({ label, value, sub }) {
  return (
    <div className="bg-white/5 px-6 py-4 border-l-2 border-white/20">
      <span className="block text-xs uppercase text-gray-500 tracking-wider mb-1">
        {label}
      </span>
      <span className="block text-3xl font-black text-white">{value}</span>
      <span className="block text-xs text-red-500 font-bold uppercase">
        {sub}
      </span>
    </div>
  );
