...
export default function OpeningDjPage() {
    ...
    const eventData = {
        ...
        time: "20:00 - 21:00",
        venue: "NCC Ground",
        video: "/events_videos/djnite.mp4",
        theme: {
            bg: "from-fuchsia-900/60 to-black",
            accent: "text-fuchsia-400",
            border: "border-fuchsia-400/30",
            buttonBg: "bg-fuchsia-500",
            glow: "shadow-[0_0_40px_rgba(232,121,249,0.4)]",
        },
        description: `Pre-loading the finale sequences. The Opening DJ sets the frequency for the last night of Ingenium 2026.

Transitioning from the tense atmosphere of the competition finals, this set is designed to bring the crowd back to operational baseline before escalating into the main DJ Night and Closing Ceremony. Expect heavy basslines and algorithmic tracking.`,
        tags: ["MUSIC", "WARMUP", "STAGE"],
        backgroundElements: (
            <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden flex items-end justify-center pb-10 gap-4">
                {/* Subwoofer pulse waves */}
                <div className="absolute top-[80%] left-1/2 -translate-x-1/2 w-[200px] h-[200px] rounded-full border border-fuchsia-500/20 animate-[ping-slow_1s_ease-out_infinite]" />
                <div className="absolute top-[80%] left-1/2 -translate-x-1/2 w-[300px] h-[300px] rounded-full border border-fuchsia-400/10 animate-[ping-slow_1s_ease-out_infinite_0.2s]" />

                {/* Fast Equalizer Bars */}
                {[...Array(10)].map((_, i) => (
                    <div
                        key={i}
                        className="w-2 bg-fuchsia-400/30 rounded-t-sm shadow-[0_0_10px_rgba(232,121,249,0.5)]"
                        style={{
                            animation: `equalizer-${(i % 3) + 1} ${Math.random() * 0.4 + 0.2}s ease-in-out infinite alternate`,
