...
export default function TechnicalExpositionPage() {
    ...
    const eventData = {
        ...
        time: "11:00 - 17:00",
        venue: "HJB Ground",
        video: "/events_videos/tech_expo.mp4",
        theme: {
        ...
        description: `Welcome to the central hub of innovation. The Technical Exposition is a sprawling showcase of hardware, software, and integrated systems developed by brilliant minds from across the nation.

Interact with working prototypes, revolutionary IoT devices, and groundbreaking AI applications. This exposition is not just a display; it's a proving ground where tomorrow's technology is tested today.`,
        tags: ["EXHIBITION", "INNOVATION", "TECHNOLOGY"],
        backgroundElements: (
            <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
                {/* Scanning laser line */}
                <div className="absolute top-0 left-0 w-full h-1 bg-cyan-400/30 shadow-[0_0_15px_rgba(34,211,238,0.8)] animate-[scanline_3s_linear_infinite]" />

                {/* Floating tech nodes */}
                {[...Array(8)].map((_, i) => (
                    <div
                        key={i}
                        className="absolute w-12 h-12 border border-cyan-400/30"
                        style={{
                            top: Math.random() * 80 + 10 + "%",
                            left: Math.random() * 80 + 10 + "%",
                            animation: `ping-slow ${Math.random() * 3 + 2}s ease-in-out infinite`,
                            animationDelay: `${Math.random() * 2}s`
                        }}
                    >
                        <div className="absolute top-1/2 left-[-20%] w-[140%] h-[1px] bg-cyan-400/20" />
                        <div className="absolute left-1/2 top-[-20%] h-[140%] w-[1px] bg-cyan-400/20" />
                    </div>
                ))}
                {/* Floating background grids */}
                <div className="absolute bottom-[20%] right-[10%] w-64 h-64 border border-cyan-500/20 rounded-full opacity-20 hover:scale-110 transition-transform duration-1000" />
            </div>
        ),
    };
