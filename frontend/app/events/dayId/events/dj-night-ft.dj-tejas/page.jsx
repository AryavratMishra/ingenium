...
export default function DjNightPage() {
    ...
    const eventData = {
        ...
        time: "21:15 - 22:30",
        venue: "NCC Ground",
        video: "/events_videos/djtejas.mp4",
        theme: {
        ...
        description: `System Override Authorized. The ultimate conclusion to Day 1. The NCC Ground transforms into a massive, pulsating open-air club.

Experience high-fidelity bass, synchronized laser setups, and a setlist guaranteed to push the crowd to its limits. This is where the code stops compiling and the energy takes over. Pure, unadulterated frequencies.`,
        tags: ["MUSIC", "PARTY", "HEADLINER"],
        backgroundElements: (
            <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden flex items-end justify-center pb-20 gap-2">
                {/* Audio Equalizer Bars */}
                {[...Array(15)].map((_, i) => (
                    <div
                        key={i}
                        className="w-4 bg-fuchsia-500/30 rounded-t-sm shadow-[0_0_15px_rgba(232,121,249,0.6)]"
                        style={{
                            animation: `equalizer-${(i % 3) + 1} ${Math.random() * 0.5 + 0.3}s ease-in-out infinite alternate`,
                ...
                {/* Strobe fx */}
                <div className="absolute inset-0 bg-fuchsia-500/5 animate-[flicker_0.2s_infinite]" />
            </div>
        ),
    };
